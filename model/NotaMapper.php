...
class NotaMapper {
	...
	*/
	public function getNoteByID($idNota){
		if(self::noteExists($idNota)){//comprobamos que la nota exista por seguridad
			...
			$nota=$stmt["0"];
			return $nota=new Nota($nota["idNota"],$nota["titulo"],$nota["contenido"],$nota["fecha"],$nota["fk_idUsuario"]);//devuelvo la �nica nota con ese id
			}
			return NULL;
		}
	...
	* Controla el tama�o del formulario para que no se desajuste el titulo en el legend
	* No se ha creado en un .js aparte por que s�lo para eso no merec�a la pena crear un .js
	*/
	public function listNote(){
		...
		foreach($notas as $nota){
			if(strlen($nota["titulo"])>13){//para que no se desajuste el tama�o de formulario
				$titulo=substr($nota["titulo"], 0, 10)."...";
			}else{
				$titulo=$nota["titulo"];
			}
			array_push($listaNotas, new Nota($nota["idNota"],$titulo["titulo"], $nota["contenido"], $nota["fecha"],$nota["fk_idUsuario"], $_SESSION["currentuser"]));
		}
		return $listaNotas;
	}
	...
	*/
	public function listShare($idUsuario){
		...
		foreach ($idCompartidas as $id) {//recorreo los ids de las notas y las obtengo
			...
			if(strlen($compartida["titulo"])>13){//para que no se desajuste el tama�o de formulario
	...
	* No se podr� editar la fecha, el autor o el id
	...
	* S�lo se llama desde la propia clase
	* Devuelve true si el autor de una nota es el actual
	* Es necesario estar logeado en el sistema si no se dar� por supuesto que no es el autor
	*/
